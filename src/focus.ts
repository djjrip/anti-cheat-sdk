...
export class FocusTracker {
  ...
  private readonly onViolation: (event: ViolationEvent) => void;
  ...
  private tick(): void {
    try {
      ...
      if (this.lastReported === procLower) return;
  ...
  private getFocusedWindow(): FocusInfo | null {
   ...
   */
  private getFocusedWindowWindows(): FocusInfo | null {
...
`.trim();
    ...
    try {
      const stdout = execSync(
        'powershell -NoProfile -NonInteractive -Command "(Get-Process | Where-Object {$_.MainWindowTitle} | Select-Object -First 1 | ForEach-Object { $_.ProcessName + \'|||\' + $_.MainWindowTitle})"',
        { encoding: 'utf-8', timeout: 5000, windowsHide: true },
      ).trim();
  ...
  private getFocusedWindowMac(): FocusInfo | null {
    try {
